...
public class BranchStep<S, E, M> extends ComputerAwareStep<S, E> implements TraversalOptionParent<M, S, E> {
    ...
    /*获取子traversal*/
    public List<Traversal> getUnionTraversals() {
    	List<Traversal> subTraversals = new ArrayList<>();
    	for(Traversal.Admin t : traversalPickOptions.get(Pick.any))
    		subTraversals.add(t);
    	return subTraversals;
    }
    
    public List<Traversal> getChooseTraversals(){
    	List<Traversal> subTraversals = new ArrayList<>();
    	subTraversals.add(getBranchTraversal());
    	subTraversals.add(getTrueTraversal());
    	subTraversals.add(getFalseTraversal());
    	return subTraversals;
    }
    private Traversal getBranchTraversal() {
    	return this.branchTraversal;
    }
    //在添加时，先加的tureTraversal后加的falseTraversal
    private Traversal getTrueTraversal() {
    	return this.traversalOptions.get(0).getValue1();
    }
    private Traversal getFalseTraversal() {
    	return this.traversalOptions.get(1).getValue1();
    }
}